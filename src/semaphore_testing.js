...
async function main() {
    ...
    const transaction_group1 = await semaphore.connect(deployer).createGroup(address_1, 16, BigInt(1), deployer.address)
    await transaction_group1.wait()
    console.log(`Group S has created`)

    const transaction_group2 = await semaphore.connect(deployer).createGroup(address_2, 16, BigInt(1), deployer.address)
    await transaction_group2.wait()
    console.log(`Group A has created`)

   
    const transaction_group3 = await semaphore.connect(deployer).createGroup(address_3, 16, BigInt(1), deployer.address)
    await transaction_group3.wait()
    console.log(`Group B has created`)

    const transaction_group4 = await semaphore.connect(deployer).createGroup(address_4, 16, BigInt(1), deployer.address)
